...
// Uses Verlet to resolve constraints easily 
    ...
    public class JiggleBone
    {
        ...
        public void VerletPass(JiggleSettingsData jiggleSettings, Vector3 wind, double time)
        {
            ...
            Vector3 localSpaceVelocity = (particleSignal.GetCurrent() - particleSignal.GetPrevious()) -
                                         (parent.particleSignal.GetCurrent() - parent.particleSignal.GetPrevious());
        ...
        {
            workingPosition = ConstrainLengthBackwards(workingPosition,
                jiggleSettings.lengthElasticity * jiggleSettings.lengthElasticity * 0.5f);
        }
        ...
        public void ConstraintPass(JiggleSettingsData jiggleSettings)
        {
            ...
            workingPosition = ConstrainAngle(workingPosition,
                jiggleSettings.angleElasticity * jiggleSettings.angleElasticity, jiggleSettings.elasticitySoften);
            workingPosition = ConstrainLength(workingPosition,
                jiggleSettings.lengthElasticity * jiggleSettings.lengthElasticity);
        }
        ...
        private Vector3 GetProjectedPosition()
        {
            ...
            return parent.transform.TransformPoint(
                parent.GetParentTransform().InverseTransformPoint(parentTransformPosition) * projectionAmount);
        }
        ...
        private Vector3 ConstrainAngleBackward(Vector3 newPosition, float elasticity, float elasticitySoften)
        {
            ...
            Vector3 cToDTargetPose =
                child.child.currentFixedAnimatedBonePosition - child.currentFixedAnimatedBonePosition;
        ...
        private Vector3 ConstrainAngle(Vector3 newPosition, float elasticity, float elasticitySoften)
        {
            ...
            {
                poseParentParent = parent.currentFixedAnimatedBonePosition +
                                   (parent.currentFixedAnimatedBonePosition - currentFixedAnimatedBonePosition);
        ...
        public void DebugDraw(Color simulateColor, Color targetColor, bool interpolated)
        {
            ...
            Debug.DrawLine(currentFixedAnimatedBonePosition, parent.currentFixedAnimatedBonePosition, targetColor, 0,
                false);
        }
        ...
        public void PoseBone(float blend)
        {
            if (child != null)
            {
                Vector3 positionBlend = Vector3.Lerp(targetAnimatedBoneSignal.SamplePosition(Time.timeAsDouble),
                    extrapolatedPosition, blend);
                Vector3 childPositionBlend =
                    Vector3.Lerp(child.targetAnimatedBoneSignal.SamplePosition(Time.timeAsDouble),
                        child.extrapolatedPosition, blend);
